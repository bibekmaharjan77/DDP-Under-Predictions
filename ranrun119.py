    ...
    for error in ERROR_VALUES:
        for frac in PREDICTION_FRACTIONS:
            for _ in range(NUM_TRIALS):
                ...
                # choose owner not in Vp; insert to keep run125 flow
                ...
                # Build Q following run125 semantics:
                # - if error == 0.0, force perfect prediction: Q = Vp
                # - else pick within a cutoff; we use diam / error (large radius for small error)
                Q = []
                if abs(error) < 1e-12:
                    Q = list(Vp)
                else:
                    cutoff = float(diam / error)
                    for v in Vp:
                        try:
                            reach = nx.single_source_shortest_path_length(G, v, cutoff=cutoff)
                            # pick a reachable node (could be v itself)
                ...
                # Max normalized distance error over pairs (Vp, Q), like run125
                max_error = 0.0
                ...
                # Stretch for every requester (skip trivial requester==owner), update owner each time
                for req in Q:
                    if req == owner:
                        continue
