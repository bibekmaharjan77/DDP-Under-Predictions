...
def simulate_halving_compare_multibend(graph_file):
    ...
    Adaptive per-batch directory:
      For each batch (error, k, trial), after sampling P we:
        1) build a 'predicted-first' node layout using P
        2) rebuild the hierarchy H with that layout
        3) (re)assign leaders from H
      Then run Q on that batch using this directory.
    """
    # --- load graph + sizes ---
    G = load_graph(graph_file)
    n = G.number_of_nodes()
    ...
    for error in ERROR_VALUES:
        for k in k_values:
            ...
            # multiple batches at this (error, k)
            for _ in range(NUM_TRIALS):
                # 1) sample predicted set P for this batch
                P = choose_Vp_halving(G, k)

                # 2) rebuild directory (hierarchy + leaders) with P-first layout
                layout  = make_pred_first_layout(list(G.nodes()), P)  # you provide this
                H       = build_mesh_hierarchy(size, layout=layout)   # accepts layout
                leaders = assign_cluster_leaders(H, seed=42)

                # publish from current owner into this batch's directory
                publish(owner, leaders)

                # 3) sample Q using the chosen error cutoff
                Q = sample_Q_within_diameter(G, P, error)

                # 4) error stats for this batch (same per-row so you can aggregate later)
                err_max, err_min, err_avg = calculate_error_stats(P, Q, G)

                # 5) run the request sequence under this directory
                for req in Q:
                    if req == owner:
                        continue

                    # our move-stretch vs MultiBend move-stretch (same directory)
