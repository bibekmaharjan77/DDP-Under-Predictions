...
def simulate_halving_compare_multibend(graph_file):
    ...
    Uses your halving counts. For each request in Q, we compute:
      - OurStr:   your current move stretch (single request)
    ...
    for error in ERROR_VALUES:
        for k in k_values:
            ...
            for _ in range(NUM_TRIALS):
                ...
                Q = sample_Q_within_diameter(G, P, error)
                # err = calculate_error(P, Q, G)
                err, err_min, err_avg = calculate_error_stats(P, Q, G)
                ...
                for req in Q:
                    ...
                    err_rate = 0.0 if error > 15 else round(1.0 / error, 1)
                    results.append((frac, err_rate, err_avg, our_str, mb_str))
...
def save_compare_results_to_excel(results, n, filename, graph_file=None):
    ...
    Sheets:
      - 'raw': one row per (request) sample
      - 'avg': mean aggregated by (Count, ErrRate)
...
def plot_mb_vs_ours_from_excel(filename, use_avg=True, err_levels=None,
    ...
    If use_avg=True it uses the 'avg' sheet; else it uses 'raw' and averages on the fly.
    """
    # read meta (to get n)
    try:
        ...
        raise ValueError("Could not read 'meta' sheet or missing 'n'.")

    # read data
    if use_avg:
        df = pd.read_excel(filename, sheet_name="avg")
        # ensure expected columns
        for col in ["Count","ErrRate","Err","OurStr","MBStr"]:
            if col not in df.columns:
                raise ValueError(f"'avg' sheet missing column: {col}")
    else:
        df = pd.read_excel(filename, sheet_name="raw")
        if "Count" not in df.columns:
            df["Count"] = (df["Frac"] * n).round().astype(int)
        df = df.groupby(["Count","ErrRate"]).mean().reset_index()

    # same plotting logic as your plot_mb_vs_ours_per_error()
    if err_levels is None:
        err_levels = ERROR_VALUES_2
    ...
    for e in err_levels:
        ...
        # Left: error vs |P|
        ax = axes[0]
        ax.plot(xvals, avg["Err"], "-o", label=f"Prediction Error ≤ {e:.1f}")
        ax.set_title("Error vs Fraction of Predicted Nodes")
        ax.set_ylabel("Average of Max Error")
        ax.set_xlabel(f"Number of predicted nodes among {n} nodes")
        if use_log_x:
        ...
        # Right: stretches
        ax = axes[1]
        ax.plot(xvals*0.985, avg["OurStr"], "-o", label=f"Our Stretch ≤ {e:.1f}", zorder=3)
        ax.plot(xvals*1.015, avg["MBStr"], "--s", label=f"MultiBend Stretch ≤ {e:.1f}", alpha=0.85, zorder=2)

        # y-axis from 0 up to max curve value
        ymax = float(np.nanmax([avg["OurStr"].max(), avg["MBStr"].max()]))
        ax.set_ylim(0, ymax * 1.05)
...
if __name__ == "__main__":
    ...
    # save the raw + averaged results to an Excel workbook
    save_compare_results_to_excel(res_cmp, n=256, filename="mb_compare_256.xlsx",
                                graph_file="256grid_diameter30test.edgelist")


    # later, plot from excel (either raw or avg sheet)
    # want the non-averaged “raw” points plotted, call with use_avg=False (the function will average on the fly before plotting)
