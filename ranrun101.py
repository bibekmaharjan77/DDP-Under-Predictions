# spiral_protocol.py — Spiral implementation for general graphs
...
def sample_requests(G, Vp, error_factor):
    ...
    Q = []
    for v in Vp:
        reach = nx.single_source_shortest_path_length(G, v, cutoff=max(1, diam // error_factor))
        Q.append(random.choice(list(reach.keys())))
    return Q

def simulate(G, hierarchy, num_trials=100, prediction_fractions=None, error_values=None):
    if prediction_fractions is None:
        ...
        for frac in prediction_fractions:
            for _ in range(num_trials):
                ...
                for req in Q:
                    ...
                    records.append({"predicted_fraction": frac, "error_rate": err_rate, "stretch": stretch})
                    owner = req
                    publish(owner, hierarchy)
...
if __name__ == "__main__":
    G = load_graph_from_edgelist("64random_diameter38test.edgelist")
    hierarchy = build_sparse_cover(G)
    df = simulate(G, hierarchy, num_trials=50)
    plot_stretch(df)
