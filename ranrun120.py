...
def get_spiral_path(node, cluster_leaders):
    # IMPORTANT: climb levels, updating 'node' to the leader each time
    path = [node]
    visited = {node}
    cur = node
    for level in sorted(cluster_leaders):
        if cur in cluster_leaders[level]:
            leader = cluster_leaders[level][cur]
...
def sample_actual_like_run125(G, Vp, eps, diam):
    """Return Q sampled so error magnitude is ≈ eps · diam around each v in Vp."""
    Q = []
    if eps <= 1e-12:
        return list(Vp)  # perfect predictions

    target = max(0, int(round(eps * diam)))  # target graph distance
    for v in Vp:
        if random.random() > eps:
            Q.append(v)
            continue

        # find nodes at distance exactly 'target' (or as close as possible below it)
        # cutoff=target ensures we only compute what we need
        reach = nx.single_source_shortest_path_length(G, v, cutoff=target)
        exact = [u for u, d in reach.items() if d == target]
        if exact:
            Q.append(random.choice(exact))
        else:
            # fall back to any within <= target (prefer max distance available)
            if reach:
                max_d = max(reach.values())
                farthest = [u for u, d in reach.items() if d == max_d]
                Q.append(random.choice(farthest))
            else:
                Q.append(v)
    return Q
    ...
    for error in ERROR_VALUES:
        for frac in PREDICTION_FRACTIONS:
            for _ in range(NUM_TRIALS):
                # Build predicted set Vp
                nodes = list(G.nodes())
                random.shuffle(nodes)
                k = max(1, int(len(nodes) * frac))
                Vp = nodes[:k]

                # Choose owner not in Vp (fallback if Vp==V)
                remaining = list(set(G.nodes()) - set(Vp))
                owner = random.choice(remaining) if remaining else random.choice(nodes)

                # Insert owner into Vp at a random position (to mirror run125 flow)
                insert_pos = random.randint(0, len(Vp))
                Vp.insert(insert_pos, owner)

                # Sample actual requesters Q with error ≈ eps·diam (run125-style)
                Q = sample_actual_like_run125(G, Vp, error, diam)

                # Publish owner’s down-links
                publish(owner, cluster_leaders)

                # Compute max normalized distance error over (Vp, Q) pairs
                max_error = 0.0
                ...
                # Measure stretch for each requester; update owner each time
                for req in Q:
                    if req == owner:
                        continue
...
def plot_results(results):
    ...
    # EXACT aggregation like run125:
    # - error: take MAX across trials for each (frac, error)
    # - stretch: take MEAN across trials for each (frac, error)
    error_agg = df.groupby(['frac', 'error'])['max_err'].max().reset_index()
    ...
    for err_val in ERROR_VALUES:
        ...
        if not subset.empty:
            subset = subset.set_index('frac').reindex(PREDICTION_FRACTIONS).reset_index()
            plt.plot(subset['frac'], subset['max_err'], 'o-', label=f'{err_val:.1f} Error')
    plt.title('Error vs Fraction of Predicted Nodes')
    plt.xlabel('Fraction of Predicted Nodes')
    plt.ylabel('Error (Max)')
    plt.xticks(PREDICTION_FRACTIONS, [f"{x:.4f}" for x in PREDICTION_FRACTIONS], rotation=45)
    plt.ylim(0, 1.05)  # start at 0; error is normalized to [0,1]
    plt.grid(True)
    ...
    for err_val in ERROR_VALUES:
        ...
        if not subset.empty:
            subset = subset.set_index('frac').reindex(PREDICTION_FRACTIONS).reset_index()
            plt.plot(subset['frac'], subset['stretch'], 'o-', label=f'{err_val:.1f} Stretch')
