...
def plot_results(df):
    ...
    ax1.set_title("Stretch vs Prediction Fraction")
    ax1.set_xlabel("Prediction Fraction")
    ax1.set_ylabel("Average Stretch")
    ax1.grid(True)
    ax1.legend(title="Error Rate")

    grouped2 = df.groupby(['error_rate', 'predicted_fraction'])['stretch'].mean().unstack()
    grouped2.plot(ax=ax2, marker='x')
    ax2.set_title("Stretch vs Error Rate")
    ax2.set_xlabel("Error Rate")
    ax2.set_ylabel("Average Stretch")
    ax2.grid(True)
    ax2.legend(title="Prediction Fraction")
