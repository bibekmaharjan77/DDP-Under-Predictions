...
# Includes: top-level cluster, spiral path diagnostics, LCA guards, and more insightful logging
...
def build_mesh_hierarchy(size):
    ...
    # Add top-level cluster (ensures common ancestor exists for all)
        ...
        for leader, cluster in leader_map[level_type]:
            if node in cluster:
                if leader not in visited:
                    path.append(leader)
                    visited.add(leader)
                break  # Only one leader per level

    # Explicitly append the root leader if not present
