...
def simulate_halving_compare_multibend(graph_file):
    """
    Compare: Our (prediction-aware) vs MB (baseline, non-prediction-aware).
    ...
    # === baseline directory once (no prediction awareness) ===
    H_base = build_mesh_hierarchy(size)                       # unchanged layout
    leaders_base = assign_cluster_leaders(H_base, seed=42)
    ...
    for error in ERROR_VALUES:
        for k in k_values:
            ...
            # start owners for both algs at the same node each batch
            owner_start = random.choice(list(G.nodes()))

            for _ in range(NUM_TRIALS):
                # --- sample predicted set P and Q for this batch ---
                P = choose_Vp_halving(G, k)
                Q = sample_Q_within_diameter(G, P, error)

                # --- prediction-aware directory for OUR algorithm (per-batch) ---
                layout = make_pred_first_layout(list(G.nodes()), P)
                H_pred = build_mesh_hierarchy(size, layout=layout)
                leaders_pred = assign_cluster_leaders(H_pred, seed=42)

                # publish owners into their respective directories
                owner_ours = owner_start
                owner_mb   = owner_start
                publish(owner_ours, leaders_pred)   # ours uses P-aware
                publish(owner_mb, leaders_base)     # MB uses baseline

                # batch error stats (repeated per-row for grouping later)
                err_max, err_min, err_avg = calculate_error_stats(P, Q, G)
                err_rate = 0.0 if error > 15 else round(1.0 / error, 1)

                # --- run the same request sequence under the two directories ---
                for req in Q:
                    if req == owner_ours:
                        # move both owners to keep next step aligned
                        owner_ours = req
                        owner_mb   = req
                        # re-publish for next step
                        publish(owner_ours, leaders_pred)
                        publish(owner_mb, leaders_base)
                        continue

                    # OUR stretch (prediction-aware directory)
                    our_str = measure_stretch_move([req], owner_ours, leaders_pred, G, trace=False)
                    owner_ours = req
                    publish(owner_ours, leaders_pred)

                    # MB stretch (baseline directory)
                ...
                # keep the two owners aligned for the *next* trial at this (error,k)
                owner_start = owner_ours  # or owner_mb; they are equal now
...
if __name__ == "__main__":
    ...
    save_compare_results_to_excel(res_cmp, n=1024, filename="mb_compare_1024.xlsx",
    ...
    # # or
    plot_mb_vs_ours_from_excel("mb_compare_1024.xlsx", use_avg=True, use_log_x=True, error_metric="ErrMax")
    # # or
