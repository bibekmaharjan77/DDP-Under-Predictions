...
def plot_stretch_minmax_avg_from_excel(filename, use_avg=True, err_levels=None,
                                       use_log_x=False, save=False,
                                       prefix="mb_vs_ours_minmax_band",
                                       error_metric="ErrAvg"):
    """
    Plots stretch with mean line and min–max whiskers PER Count (|P|).
    Reads the 'raw' sheet to compute per-Count min/mean/max across trials.
    Arguments kept for API compatibility; 'use_avg' and 'error_metric' are ignored.
    """
    ...
    # --- always use RAW so we can compute min/mean/max across trials ---
    raw = pd.read_excel(filename, sheet_name="raw")
    if "Count" not in raw.columns:
        raw["Count"] = (raw["Frac"] * n).round().astype(int)

    if err_levels is None:
        # default to your globals if available; else infer
        try:
            levels = ERROR_VALUES_2
        except NameError:
            levels = sorted(raw["ErrRate"].unique())
        err_levels = levels
    ...
    for e in err_levels:
        ...
        # Per-|P| (Count) stats across trials
        agg = (sub.groupby("Count", as_index=False)
                  .agg(OurMean=("OurStr", "mean"),
                       OurMin =("OurStr", "min"),
                       OurMax =("OurStr", "max"),
                       MBMean =("MBStr", "mean"),
                       MBMin  =("MBStr", "min"),
        ...
        x = agg["Count"].to_numpy(dtype=float)

        # y and whiskers for Our
        y_ours      = agg["OurMean"].to_numpy()
        yerr_ours   = np.vstack([y_ours - agg["OurMin"].to_numpy(),
                                 agg["OurMax"].to_numpy() - y_ours])

        # y and whiskers for MB
        y_mb        = agg["MBMean"].to_numpy()
        yerr_mb     = np.vstack([y_mb - agg["MBMin"].to_numpy(),
                                 agg["MBMax"].to_numpy() - y_mb])

        fig, ax = plt.subplots(1, 1, figsize=(7.5, 5.2), constrained_layout=True)

        # mean line + min/max whiskers
        ax.errorbar(x*0.985, y_ours, yerr=yerr_ours, fmt='-o',
                    label=f"PMultiBend (err ≤ {e:.1f})", capsize=5, zorder=3)
        ax.errorbar(x*1.015, y_mb,   yerr=yerr_mb,   fmt='--s',
                    label="MultiBend", alpha=0.9, capsize=5, zorder=2)

        # y limits
        ymax = float(np.nanmax([agg["OurMax"].max(), agg["MBMax"].max()]))
        ax.set_ylim(0, ymax * 1.05)

        # x axis: log2 optional
        if use_log_x:
            try:
            ...
            # pretty power-of-two tick labels if they are exact powers
            def _p2_fmt(val, _pos):
                # show as 2^k if val is power of two; else raw int
                k = np.log2(val)
                return f"$2^{{{int(k)}}}$" if abs(k - round(k)) < 1e-9 else f"{int(val)}"
            ax.xaxis.set_major_formatter(FuncFormatter(_p2_fmt))
        ...
        ax.grid(True, axis="y", alpha=0.35)
        ax.legend(loc="best")
        # ax.set_title(f"Stretch with per-|P| min–max whiskers (err ≤ {e:.1f})")
