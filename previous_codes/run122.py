# trying to debug using gpt line-by-line
...
def measure_stretch(requesters, owner, leader_map, G):
    ...
    for r in requesters:
        ...
        if DEBUG:  
            print("\n  >> Request", r)
            print("     Up‐path:", sp[:i+2],   # sp up to intersection
                  f"(hops up={up_hops})")
            print("     Intersection at level‐leader:", intersection)
            print("     Down‐links table:", down_link)
            print("     Down‐path hops:", down_hops)
            print("     True shortest‐path hops:", dist_opt)
            print("     Stretch this request:", dist_sp/dist_opt)
...
def hop_dist(a,b):
    # prints whether one-bend or two-bend
    (x1,y1),(x2,y2) = a,b
    hops = abs(x1-x2)+abs(y1-y2)
    bends = 1 if (x1!=x2 and y1!=y2) else 0
    print(f"      hop {a}->{b}: dist={hops}, {'one' if bends==0 else 'two'}-bend")
    return hops
...
# using 2D grid API so that the nodes are in (x,y) format
def load_graph(size):
    G = nx.grid_2d_graph(size, size)    # nodes = (i,j) for i,j in [0..size)
    # if you need weights:
    for u,v in G.edges():
        G[u][v]['weight'] = 1
    return G
...
def simulate(graph_size):
    # G = load_graph(graph_file)
    G = load_graph(graph_size)
    size = int(math.sqrt(G.number_of_nodes()))
    H = build_mesh_hierarchy(size)
    leaders = assign_cluster_leaders(H)

    # added to see whats the results
    print("\n=== CLUSTER HIERARCHY ===")
    for (lvl, typ) in sorted(H):
        print(f" Level {(lvl,typ)} (size={len(H[(lvl,typ)])} clusters):")
        for i, cluster in enumerate(H[(lvl,typ)]):
            lead = min(cluster)
            print(f"   #{i:2d}: leader={lead}, nodes={sorted(cluster)}")


    results = []
    owner = (0,0)
    requesters = [(3,1)]   # just one for now
    # build our one directory once
    publish(owner, leaders)

    print(f"\n--- Single DEBUG run: requester={requesters[0]}, owner={owner} ---")
    stretch = measure_stretch(requesters, owner, leaders, G)
    print(f"FINAL STRETCH = {stretch:.4f}")
