...
def simulate_multibend_eval(graph_file, output_excel="excel/results.xlsx"):
    """
    ...
    df.to_excel(os.path.join("excel", output_excel), index=False)
    return output_excel  # Return the path to the saved Excel file
...
def main():
    ...
    Entry point for executing the full evaluation pipeline:
      ...
      - Plots results
    """
    excel_path = simulate_multibend_eval("64grid_diameter14test.edgelist", output_excel="64grid_diameter14test.xlsx")
    plot_results(os.path.join("excel", excel_path))
