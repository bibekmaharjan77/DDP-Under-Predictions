# not fully random - maintains overlap for controlled error
# like 0.0 (prediction fully correct - 100% overlap 
# 0.1, 0.2 till 0.5
...
def calculate_stretch_spiral(pred_nodes, actual_nodes, leader_map, G):
    ...
    for u, v in zip(pred_nodes, actual_nodes):
        ...
        if dist_shortest == 0:
            if DEBUG:
                print(f"[WARN] Shortest path from {u} to {v} is zero (same node), skipping.")
            continue
