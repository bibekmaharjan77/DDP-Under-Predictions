...
def plot_results(df):
    ...
    for error in sorted(df["error_value"].unique()):
        subset = df[df["error_value"] == error]
        means = subset.groupby("predicted_fraction").agg({"error_rate": "mean", "stretch": "mean"})
        ax1.plot(means.index, means["error_rate"], marker='o', label=f"{error:.1f}")
        ax2.plot(means.index, means["stretch"], marker='x', label=f"{error:.1f}")
