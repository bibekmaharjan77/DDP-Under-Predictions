# tried to implement the corrected error plots from sush's code
...
def print_cluster_summary(hierarchy):
    print("\n Cluster Summary (per level):")
    for (level, ctype), clusters in sorted(hierarchy.items()):
        print(f"  Level {level}, Type {ctype}: {len(clusters)} clusters")
...
def simulate_multibend_eval(graph_file):
    ...
    For each error rate and prediction fraction:
      ...
      - Saves results into Excel for external plotting
    """
    ...
    print_cluster_summary(hierarchy)   # Print a summary of the clusters in the hierarchy
    ...
    for error_rate in ERROR_VALUES:
        ...
        for frac in PREDICTION_FRACTIONS:
            ...
            actual_nodes = sample_Q_within_diameter(G, pred_nodes, error_rate)  # Sample actual nodes (Q) based on the predicted nodes and error rate
            
            # Removed in sush's code: num_nodes = int(frac * len(G))   # Calculate the number of nodes to sample based on the fraction

            err = calculate_error_updated(pred_nodes, actual_nodes, G)

            # Removed in sush's code
            ...
            #     remaining_pool = [n for n in remaining_pool if n not in remaining_pred]
            ...
            # Removed in sush's code: results.append((frac, error_rate, trial, err, stretch))
            if error_rate > 15:
