...
def plot_stretch_minmax_avg_from_excel(filename, use_avg=True, err_levels=None,
                                       use_log_x=False, save=False, prefix="mb_vs_ours_minmax_band",
                                       error_metric="ErrAvg"):
    from matplotlib.ticker import FormatStrFormatter
    import pandas as pd
    import numpy as np
    ...
    # Read data depending on whether we want averages or raw data
    if use_avg:
        df = pd.read_excel(filename, sheet_name="avg")
    else:
        raw = pd.read_excel(filename, sheet_name="raw")
    ...
    # Set default error levels if not provided
    if err_levels is None:
        err_levels = ERROR_VALUES_2
    ...
    for e in err_levels:
        ...
        print(f"ErrRate: {e}, Avg stretch values:\n{avg[['OurStr', 'MBStr']]}")

        # ---- Boxplot and Whisker Plot ----
        # Compute the min, max, and avg stretch values for error bars
        min_ours = avg["OurStr"].min()
        max_ours = avg["OurStr"].max()
        min_mb = avg["MBStr"].min()
        max_mb = avg["MBStr"].max()

        print(f"Min/Max OurStr: {min_ours}, {max_ours}")
        print(f"Min/Max MBStr: {min_mb}, {max_mb}")

        # Calculate the difference from the min/max for each data point (for error bars)
        lower_ours = avg["OurStr"] - min_ours
        upper_ours = max_ours - avg["OurStr"]

        lower_mb = avg["MBStr"] - min_mb
        upper_mb = max_mb - avg["MBStr"]

        # Plot the average line with error bars (min, max) for both methods
        ax.errorbar(x*0.985, avg["OurStr"], yerr=[lower_ours, upper_ours], fmt='-o', label=f"PMultiBend (err ≤ {e:.1f})", 
                    color="blue", zorder=3, capsize=4)
        ax.errorbar(x*1.015, avg["MBStr"], yerr=[lower_mb, upper_mb], fmt='--s', label="MultiBend", 
                    color="orange", alpha=0.85, zorder=2, capsize=4)
        ...
        ax.set_title(f"Stretch vs |P| with min–max error bands (err ≤ {e:.1f})")
