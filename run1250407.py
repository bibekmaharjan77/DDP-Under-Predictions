...
def single_request_move_cost(r, owner, leader_map, G, weight=None, trace=False):
    """
    Return total 'move' cost for a single request r:
      UP + DOWN + OBJ
    using the same logic as in measure_stretch_move / multibend_move_sequence_stretch,
    but without updating the owner.
    """
    up, down, obj = _single_request_costs(r, owner, leader_map, G, weight, trace)
    return up + down + obj
...
def simulate_halving_compare_multibend(graph_file):
    ...
    Returns rows: (Frac, ErrRate, ErrMax, ErrMin, ErrAvg, OurStr, MBStr, OurCostDiff).
    OurCostDiff is (cost(q) - cost(p)) for our scheme, per request.
    """
    G = load_graph(graph_file)
    n = G.number_of_nodes()
    size = int(math.sqrt(n))
        ...
        for k in k_values:
            ...
            for _ in range(NUM_TRIALS):
                ...
                for idx, req in enumerate(Q):
                    p = P[idx]  # predicted node paired with this request

                    if req == owner_ours:
                        # object already at requester: no real move cost; keep owners in sync
                        owner_ours = req
                        owner_mb   = req
                        publish(owner_ours, leaders_pred)
                        publish(owner_mb,   leaders_base)
                    ...
                    # ---- NEW: prediction cost difference for OUR directory ----
                    # cost if actual requester is q = req
                    cost_q_ours = single_request_move_cost(
                        req, owner_ours, leaders_pred, G, weight=None, trace=False
                    )
                    # cost if we instead had to serve predicted node p
                    cost_p_ours = single_request_move_cost(
                        p, owner_ours, leaders_pred, G, weight=None, trace=False
                    )
                    our_cost_diff = cost_q_ours - cost_p_ours
                    # (positive means: actual request was more expensive than predicted)

                    # ---- OLD: stretch computation (unchanged logic) ----
                    ...
                    # now we store our_cost_diff as an extra field in results
                    results.append((frac, err_rate, err_max, err_min, err_avg,
                                    our_str, mb_str, our_cost_diff))
...
def save_compare_results_to_excel(results, n, filename, graph_file=None):
    ...
    Sheets:
      ...
      - 'avg': max aggregated by (Count, ErrRate) for all metrics
      - 'meta': run metadata (n, graph, trials, timestamp, etc.)
    """
    cols = ["Frac","ErrRate","ErrMax","ErrMin","ErrAvg","OurStr","MBStr","OurCostDiff"]
    df   = pd.DataFrame(results, columns=cols)
    df["Count"] = (df["Frac"] * n).round().astype(int)

    # group by (Count, ErrRate) and take the max for 'OurCostDiff' instead of the mean
    group_cols = ["Count", "ErrRate"]
    avg = df.groupby(group_cols, as_index=False).agg({
        "ErrMax": "max",
        "ErrMin": "min",
        "ErrAvg": "mean",
        "OurStr": "mean",  # Stretch mean
        "MBStr": "mean",   # MB Stretch mean
        "OurCostDiff": "max"  # Max of OurCostDiff
    })
...
def plot_mb_vs_ours_from_excel(filename, use_avg=True, err_levels=None,
    ...
    if use_avg:
        ...
        df = raw.groupby(["Count", "ErrRate"], as_index=False).agg({
            "ErrMax": "max",
            "ErrMin": "min",
            "ErrAvg": "mean",
            "OurStr": "mean",  # Stretch mean
    ...
    for e in err_levels:
        ...
        if use_log_x:
            ...
            except TypeError: ax2.set_xscale("log", basex=2)
        ax2.set_xticks(x)
